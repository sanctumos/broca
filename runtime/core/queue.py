    ...
    async def _process_single_message(self, queue_item: Any) -> None:
        ...
        try:
            ...
            if self.message_mode == "echo":
                ...
                # plus attach/detach/network. If this fires first, asyncio cancels the turn while
                # Letta may still be running; Broca then requeues → same user text POST'd again
                # (duplicate assistant activity / repeated retries on long jobs).
                long_task_max = int(get_env_var("LONG_TASK_MAX_WAIT", default="600"))
                outer_buffer = int(get_env_var("MESSAGE_PROCESS_TIMEOUT_BUFFER", default="180"))
