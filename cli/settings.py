...
def set_message_mode(args) -> None:
    ...
    if args.mode not in valid_modes:
        error_msg = (
            f"Error: Invalid message mode. Must be one of: {', '.join(valid_modes)}"
        )
...
def main():
    ...
    mode_parser.add_argument(
        "mode", choices=["echo", "listen", "live"], help="Set message mode"
    )
    ...
    debug_group.add_argument(
        "--disable", action="store_false", dest="enable", help="Disable debug mode"
    )

    # Queue refresh commands
    refresh_parser = subparsers.add_parser("refresh", help="Queue refresh management")
    refresh_parser.add_argument(
        "seconds", type=int, help="Queue refresh interval in seconds"
    )
