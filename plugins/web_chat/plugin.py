...
class WebChatPlugin(Plugin):
    ...
    def __init__(self, settings: WebChatSettings | None = None):
        self.settings = settings  # Initialize lazily
        self.api_client: WebChatAPIClient | None = None
        self.message_handler: WebChatMessageHandler | None = None
    ...
    async def _poll_messages(self):
        ...
        self.logger.info(
            f"Starting message polling with {self.settings.poll_interval}s interval"
        )
    ...
    async def _process_message(self, message_data: dict[str, Any]):
        ...
        try:
            ...
            if message:
                ...
                self.logger.info(
                    f"Message queued for processing from session {message_data.get('session_id')}"
                )
    ...
    async def send_response(
        self, session_id: str, response_text: str, original_message=None
    ) -> bool:
        """
    ...
    async def _handle_response(self, response: str, profile, message_id: int) -> None:
        ...
        try:
            ...
            if not session_id:
                self.logger.error(
                    f"No session_id found in profile metadata for message {message_id}"
                )
                return
    ...
    async def handle_agent_response(
        self, agent_response: str, session_id: str, original_message=None
    ) -> bool:
        """
