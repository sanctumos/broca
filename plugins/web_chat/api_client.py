...
class WebChatAPIClient:
    ...
    def _get_headers(self) -> dict[str, str]:
        ...
        self.logger.debug(
            f"🔍 API Key: {self.settings.api_key[:10]}..."
            if self.settings.api_key
            else "🔍 API Key: None/Empty"
        )
        self.logger.debug(f"🔍 Request Headers: {headers}")
        return headers

    async def get_messages(
        self, limit: int = 50, offset: int = 0, since: str | None = None
    ) -> list[dict[str, Any]]:
        """
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    if data.get("success"):
                        ...
                        self.logger.info(
                            f"Retrieved {len(messages)} messages from web chat API"
                        )
                        return messages
                    else:
                        self.logger.error(
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    if response_data.get("success"):
                        self.logger.info(
                            f"Successfully posted response to session {session_id}"
                        )
                        return True
                    else:
                        self.logger.error(
                            f"Failed to post response: {response_data.get('message', 'Unknown error')}"
                        )
                        return False
                else:
                    self.logger.error(
                        f"Failed to post response, status {response.status}"
                    )
                    return False
        except Exception as e:
