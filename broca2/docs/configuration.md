...
Located in the root `broca2/` directory, these settings are shared across all agent instances:
...
- `broca2/settings.json`: Main runtime settings (log level, max workers, timeout, etc.)
- `broca2/.env`: Base environment variables for shared services
- `broca2/requirements.txt`: Python dependencies for all agents

### Agent Instance Files
- `broca2/agent-{uuid}/.env`: Agent-specific environment variables
- `broca2/agent-{uuid}/settings.json`: Agent-specific runtime settings
- `broca2/agent-{uuid}/sanctum.db`: Agent-specific database
- `broca2/agent-{uuid}/logs/`: Agent-specific log directory
...
### Base Settings (`broca2/settings.json`)
```json
{
  "system": {
    "log_level": "INFO",
...
Plugins can define base settings in `broca2/settings.json`:
```json
{
  "plugins": {
    "telegram": {
...
3. **Base settings** (`broca2/settings.json`)
4. **Base environment** (`broca2/.env`)
...
1. Add the new option to `broca2/settings.json` or `broca2/.env`
