...
## Where this lives (ships with Broca)

This plugin is part of the **Broca** repo at **`smcp/broca/`**. **sanctumos/smcp** does not ship it. Set **`MCP_PLUGINS_DIR`** to the **`smcp`** directory inside your Broca checkout (or a deployment copy):

```bash
export MCP_PLUGINS_DIR=/path/to/sanctumos/broca/smcp
```

SMCP scans **`MCP_PLUGINS_DIR`** for subdirectories containing **`cli.py`**. The folder name **`broca`** registers tools as **`broca__…`**.
...
|----------|-------------|
| **`MCP_PLUGINS_DIR`** | Parent of this plugin’s folder: set to **`…/broca/smcp`** (this repo’s `smcp` dir), not `…/broca/smcp/broca`. |
...
# Point at Broca repo’s smcp/ dir (contains broca/cli.py), not the old SMCP vendor tree
export MCP_PLUGINS_DIR=/home/USER/sanctum/agents/rico/broca/smcp
...
python3 /path/to/sanctumos/broca/smcp/broca/cli.py --describe | head
```
...
Tools register as **`broca__<command>`**, e.g. `broca__queue_list`, `broca__send_outbound`. Full list: run **`python smcp/broca/cli.py --describe`** from the Broca repo root (JSON).
